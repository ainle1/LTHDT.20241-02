...
public class EmulatorController {
    ...
    @FXML
    private Label energyLabel;  // Liên kết với Label năng lượng
    ...
    public void setup(String mode) {
        ...
        if (ecosystemValues.length == 3) {
            ...
            energyLabel.textProperty().bind(hesinhthai.energyProperty().asString());
 ...
 // Phương thức xử lý khi nhấn vào Stop Resume Button
    ...
    // Phương thức handleSetting
    @FXML
    private void handleSetting() {
        // Gọi phương thức dungLai() trước khi mở Alert
        hesinhthai.dungLai();

        // Tạo một alert kiểu INFORMATION với layout tùy chỉnh
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        
        // Đặt tiêu đề cho alert
        alert.setTitle("Settings");

        // Xóa icon mặc định của alert
        alert.setGraphic(null);

        // Tạo một VBox để chứa văn bản và trường nhập liệu
        VBox vbox = new VBox(10);
        
        // Thêm văn bản yêu cầu người dùng nhập tham số
        Label instructionLabel = new Label("Hãy nhập tham số điều chỉnh hệ sinh thái");

        // Thêm tên trường "Hiệu suất chuyển đổi năng lượng :" ở phía trên trường input
        Label inputLabel = new Label("Hiệu suất chuyển đổi năng lượng :");
        
        // Tạo một TextField cho trường nhập liệu
        TextField input = new TextField();
        input.setPromptText("Nhập từ 1 - 100");  // Giới hạn giá trị nhập vào từ 1 - 100
        
        // Thêm các Label và TextField vào VBox
        vbox.getChildren().addAll(instructionLabel, inputLabel, input);

        // Đặt VBox vào content của alert
        alert.getDialogPane().setContent(vbox);

        // Đặt tiêu đề của alert
        alert.setHeaderText(null);  // Không có tiêu đề (tùy chọn)

        // Cấu hình sự kiện khi người dùng nhấn nút "OK"
        alert.setOnHidden(e -> {
            try {
                // Lấy giá trị từ TextField và chuyển thành số
                ...
                double energyrate;
                if (number > 100) {
                    energyrate = 1.0;  // Nếu lớn hơn 100, đặt là 1.0
                } else {
                    energyrate = number / 100.0;  // Chuyển thành tỉ lệ phần trăm
                }

                // Gọi phương thức setEnergyrate với giá trị đã tính
                hesinhthai.setEnergyrate(energyrate);

                // In ra giá trị (chỉ để debug)
                System.out.println("Energy rate set to: " + energyrate);
            } catch (NumberFormatException ex) {
                // Xử lý lỗi khi người dùng không nhập số hợp lệ
                System.out.println("Vui lòng nhập một số hợp lệ.");
            }

            // Sau khi alert bị đóng hoặc xác nhận, gọi tieptuc()
            hesinhthai.tieptuc();
        });

        // Hiển thị alert và đợi người dùng tương tác
        alert.showAndWait();
    }
